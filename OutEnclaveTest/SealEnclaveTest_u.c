...
typedef struct ms_AggregateRows_SGX_t {
	char* ms_buf;
	size_t ms_len;
	int ms_trainingSize;
	int ms_precision;
} ms_AggregateRows_SGX_t;

typedef struct ms_ScaleDown_SGX_t {
	char* ms_buf;
	size_t ms_len;
	int ms_trainingSize;
	int ms_precision;
...
typedef struct ms_HalfScaleDown_SGX_t {
	char* ms_buf;
	size_t ms_len;
	int ms_trainingSize;
	int ms_precision;
} ms_HalfScaleDown_SGX_t;
...
static const struct {
	...
	5,
	{
		(void*)(uintptr_t)SealEnclaveTest_sgx_oc_cpuidex,
		(void*)(uintptr_t)SealEnclaveTest_sgx_thread_wait_untrusted_event_ocall,
		(void*)(uintptr_t)SealEnclaveTest_sgx_thread_set_untrusted_event_ocall,
		(void*)(uintptr_t)SealEnclaveTest_sgx_thread_setwait_untrusted_events_ocall,
		(void*)(uintptr_t)SealEnclaveTest_sgx_thread_set_multiple_untrusted_events_ocall,
	}
};
...
sgx_status_t DecreaseNoise_SGX(sgx_enclave_id_t eid, char* buf, size_t len, int trainingSize, double y_initial)
{
	sgx_status_t status;
...
sgx_status_t AggregateRows_SGX(sgx_enclave_id_t eid, char* buf, size_t len, int trainingSize, int precision)
{
	sgx_status_t status;
	ms_AggregateRows_SGX_t ms;
	ms.ms_buf = buf;
	ms.ms_len = len;
	ms.ms_trainingSize = trainingSize;
	ms.ms_precision = precision;
...
sgx_status_t ScaleDown_SGX(sgx_enclave_id_t eid, char* buf, size_t len, int trainingSize, int precision)
{
	sgx_status_t status;
	ms_ScaleDown_SGX_t ms;
	ms.ms_buf = buf;
	ms.ms_len = len;
	ms.ms_trainingSize = trainingSize;
	ms.ms_precision = precision;
	status = sgx_ecall(eid, 9, &ocall_table_SealEnclaveTest, &ms);
	return status;
}

sgx_status_t HalfScaleDown_SGX(sgx_enclave_id_t eid, char* buf, size_t len, int trainingSize, int precision)
{
	sgx_status_t status;
	ms_HalfScaleDown_SGX_t ms;
	ms.ms_buf = buf;
	ms.ms_len = len;
	ms.ms_trainingSize = trainingSize;
	ms.ms_precision = precision;
	status = sgx_ecall(eid, 10, &ocall_table_SealEnclaveTest, &ms);
	return status;
}
