...
static sgx_status_t SGX_CDECL sgx_DecreaseNoise_SGX(void* pms)
{
	...
	DecreaseNoise_SGX(_in_buf, _tmp_len, ms->ms_trainingSize, ms->ms_y_initial);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
static sgx_status_t SGX_CDECL sgx_AggregateRows_SGX(void* pms)
{
	ms_AggregateRows_SGX_t* ms = SGX_CAST(ms_AggregateRows_SGX_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_buf = ms->ms_buf;
	size_t _tmp_len = ms->ms_len;
	size_t _len_buf = _tmp_len;
	...
	CHECK_REF_POINTER(pms, sizeof(ms_AggregateRows_SGX_t));
	CHECK_UNIQUE_POINTER(_tmp_buf, _len_buf);
	...
	AggregateRows_SGX(_in_buf, _tmp_len, ms->ms_trainingSize, ms->ms_precision);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
static sgx_status_t SGX_CDECL sgx_ScaleDown_SGX(void* pms)
{
	ms_ScaleDown_SGX_t* ms = SGX_CAST(ms_ScaleDown_SGX_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_buf = ms->ms_buf;
	size_t _tmp_len = ms->ms_len;
	size_t _len_buf = _tmp_len;
	...
	CHECK_REF_POINTER(pms, sizeof(ms_ScaleDown_SGX_t));
	CHECK_UNIQUE_POINTER(_tmp_buf, _len_buf);
	...
	ScaleDown_SGX(_in_buf, _tmp_len, ms->ms_trainingSize, ms->ms_precision);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
static sgx_status_t SGX_CDECL sgx_HalfScaleDown_SGX(void* pms)
{
	ms_HalfScaleDown_SGX_t* ms = SGX_CAST(ms_HalfScaleDown_SGX_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_buf = ms->ms_buf;
	size_t _tmp_len = ms->ms_len;
	size_t _len_buf = _tmp_len;
	...
	CHECK_REF_POINTER(pms, sizeof(ms_HalfScaleDown_SGX_t));
	CHECK_UNIQUE_POINTER(_tmp_buf, _len_buf);
	...
	HalfScaleDown_SGX(_in_buf, _tmp_len, ms->ms_trainingSize, ms->ms_precision);
err:
	if (_in_buf) {
		memcpy(_tmp_buf, _in_buf, _len_buf);
		free(_in_buf);
...
SGX_EXTERNC const struct {
	...
	11,
	{
		...
		{(void*)(uintptr_t)sgx_AggregateRows_SGX, 0},
		{(void*)(uintptr_t)sgx_ScaleDown_SGX, 0},
		{(void*)(uintptr_t)sgx_HalfScaleDown_SGX, 0},
	}
};
